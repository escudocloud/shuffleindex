...
public Key splitNodeInMemory(ArrayList<Node<Key, Value>> ns, DiskSuperBlock sup) {
		...
		tref.num = mid;
		for (i = 0; i<mid; i++) tref.values[i] = this.values[i];
		
		tref = (LNode<Key, Value>) ns.get(1);
		tref.pid = sup.getNewNodeOffsets(0); tref.vid = tref.pid;
		for (i = mid; i<num; i++) tref.keys[i-mid] = this.keys[i];
		tref.num = num-mid;
		for (i = mid; i<num; i++) tref.values[i-mid] = this.values[i];
